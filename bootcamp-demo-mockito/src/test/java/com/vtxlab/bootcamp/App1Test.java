...
public class App1Test {
  ...
  void testFirstCharacters() {
    ...
    when(dependency.encode('j')).thenReturn('K');
    App1 app1 = new App1(dependency);
    List<String> strings = new ArrayList<>(List.of("abc", "ijk", "cba", "jba"));
    String result = app1.firstCharacters(strings);
    assertEquals("BJDK", result);

    // verify()
    verify(dependency, times(1)).equals('a');
    verify(dependency, times(1)).equals('i');
    verify(dependency, times(1)).equals('c');
    verify(dependency, times(2)).equals('j');
  }
